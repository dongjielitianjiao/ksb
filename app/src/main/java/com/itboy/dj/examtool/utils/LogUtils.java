...
public class LogUtils {
 ...
 /*   *分段打印出较长log文本
    ...
    *
    @param
    showLength 规定每段显示的长度
    （AndroidStudio控制台打印log的最大信息量大小为4k）
            *
    @param
    tag 打印log的标记
    */
    ...
    public static void showLargeLong(String logContent) {

        if (logContent.length() > 4000) {

            Log.v(TAG, "sb.length = " + logContent.length());

            int chunkCount = logContent.length() / 4000;     // integer division

            for (int i = 0; i <= chunkCount; i++) {

                int max = 4000 * (i + 1);

                if (max >= logContent.length()) {

                    Log.v(TAG, "chunk " + i + " of " + chunkCount + ":" + logContent.substring(4000 * i));

                } else {

                    Log.v(TAG, "chunk " + i + " of " + chunkCount + ":" + logContent.substring(4000 * i, max));
